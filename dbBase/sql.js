const sql = {
  ...
  getPublicNoteListSql: 'select * from (select * from note_list where type="public" and openid=? order by noteTime asc limit ?,?)as a left join(select * from account )as b on a.openid=b.openid',
  getNoteListAllSql: 'select * from (select * from note_list where type=? order by noteTime asc limit ?,?)as a left join(select * from account )as b on a.openid=b.openid',
  getNoteListPrivateSql: 'select * from (select * from note_list where type=? and openid=? order by noteTime asc limit ?,?)as a left join(select * from account )as b on a.openid=b.openid',
